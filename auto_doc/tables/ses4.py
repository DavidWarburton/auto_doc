...
class Ses4(TrackedTable):
    ...
    def build(self, cursor=None):
        ...
        cur.execute("""
                    create temp table ses_temp as
                    select a.study_id,
                    demographics_unduped_gender , a.deaths_unduped_date_of_death, 
                    demographics_unduped_dob ,
                    merge_sub_keep_0(merge_pattern,msp_subsidy_pat_pat ) as ses_pat
                    from temp_ses_2 a, msp_subsidy_pat  b
                    where a.study_id = b.study_id ; 
                    """)
                    
        cur.execute("""
                    create table ses4 as
                    select a.study_id,
                    demographics_unduped_gender,deaths_unduped_date_of_death,
                    demographics_unduped_dob,
                    merge_non_reg(ses_pat,rpb_non_reg_pat ) as ses4_pat
                    from ses_temp a left outer join rpb_non_reg  b
                    on a.study_id = b.study_id ; 
                    """)
