...
class LeftProvinceYearsAgoByDate(TrackedTable):
    name = 'left_province_years_ago_by_date'

    def build(self, cursor=None):
        if not cursor:
            cursor = self.get_cursor()
        cursor.execute("""
            CREATE TABLE left_province_years_ago_by_date AS 
                SELECT COUNT(*), 
            ...
            WHERE 
                ...
                b.all_rpb_cancel_cancel_reason = 'W' OR 'Y' 
            GROUP BY 2, 3
        """)
